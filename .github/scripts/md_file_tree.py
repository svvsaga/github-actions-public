...
def create_index(cwd, headings=False, wikilinks=False):
    ...
    for root, dirs, files in os.walk(cwd):
        files = sorted([f for f in files if not f[0] ==
                       '.' and os.path.splitext(f)[-1] in md_exts]) if root != cwd else []
        ...
        if len(files) > 0:
            ...
            for md_filename in files:
                ...
                if wikilinks:
                    ...
                    md_titlename = md_filename.replace(".md", "")
                    md_titlename = re.split("_|\-", md_titlename)
                    md_titlename = " ".join(md_titlename).capitalize()
                    md_lines.append('{0} {4} [{1}]({3}{2})\n'.format(indent,
                                                                     md_titlename,
                                                                     md_filename,
                                                                     rel_dir,
                                                                     TOC_LIST_PREFIX))
                if headings:
                    ...
                    if len(results) > 0:
                        ...
                        for header in results:
                            header_level = header[0] - \
                                min_header_level + level + 2
                            indent = '  ' * header_level
                            md_lines.append("{}{} {}\n".format(
                                indent, TOC_LIST_PREFIX, header[1]))
